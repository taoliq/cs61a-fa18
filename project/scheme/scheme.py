...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while cur_formal and cur_val:
            new_frame.define(cur_formal.first, cur_val.first)
            cur_formal, cur_val = cur_formal.second, cur_val.second
        return new_frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        procedure = LambdaProcedure(target.second, expressions.second, env)
...
def do_and_form(expressions, env):
    ...
    val = True
    while cur_exp:
        if cur_exp.second == nil:
            return scheme_eval(cur_exp.first, env, True)
        val = scheme_eval(cur_exp.first, env)
        if scheme_falsep(val):
            return val
        cur_exp = cur_exp.second
    return val
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            return eval_all(clause.second, env, True) if clause.second else test
            # END PROBLEM 14
...
def make_let_frame(bindings, env):
    ...
    def helper(binds):
        if binds is nil:
            return nil, nil
        check_form(binds.first, 2, 2)
        symbols_rest, values_rest = helper(binds.second)
        return Pair(binds.first.first, symbols_rest), \
                Pair(scheme_eval(binds.first.second.first, env), values_rest)
    symbols, values = helper(bindings)
