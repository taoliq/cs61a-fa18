...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return max(score // 10 * 2 - score % 10, 1)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    player_score %= 100
    opponent_score %= 100
    return abs(player_score // 10 - player_score % 10) \
            == abs(opponent_score // 10 - opponent_score % 10)
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score[0] < goal and score[1] < goal:
        ...
        num_rolls = strategy[cur_turn](score[cur_turn], score[other(cur_turn)])
        score[cur_turn] += take_turn(num_rolls, score[other(cur_turn)], dice)
        if is_swap(score[cur_turn], score[other(cur_turn)]):
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        score = score0 if not who else score1
        diff = score - previous_score
        high = max(previous_high, diff)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    roll_simulate = make_averaged(roll_dice, num_samples)
    simulate_result = [roll_simulate(i + 1, dice) for i in range(10)]
    return simulate_result.index(max(simulate_result)) + 1
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score + gain, opponent_score) and score + gain < opponent_score:
        return 0
    if gain >= margin and \
            not (is_swap(score + gain, opponent_score) and score + gain > opponent_score):
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score + bacon > 90 and not is_swap(score + bacon, opponent_score):
        return 0
