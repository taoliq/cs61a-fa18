...
def find_centroid(cluster):
    ...
    lati_long = zip(*locations)
    return [mean(lati_long[0]), mean(lati_long[1])]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictor_r = [find_predictor(user, reviewed, f) for f in feature_fns]
    return max(predictor_r, key=lambda x: x[1])[0]
    # END Question 8
