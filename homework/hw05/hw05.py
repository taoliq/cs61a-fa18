...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        ...
        if input_password != password:
            incorrect_attempts.append(input_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def new_withdraw(amount, input_password):
        pw = old_password if input_password == new_password else input_password
...
def multiple_references_explanation():
    ...
    Eva is right. When a uncertain number is computed, the interval span will extend.
    The less these number is used, the tighter the error bounds would be.

    The program par2 is better than par1 because these number is used only once 
    which is less than program par1.
    """
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Offical solution is so beautiful
    extremum = -b / (2*a)
    f = lambda x: a*x*x + b*x +c
    lo, up, ex = map(f, (lower_bound(x), upper_bound(x), extremum))
    if lower_bound(x) <= extremum <= upper_bound(x):
        return interval(min(lo, up, ex), max(lo, up, ex))
    else:
        return interval(min(lo, up), max(lo, up))
