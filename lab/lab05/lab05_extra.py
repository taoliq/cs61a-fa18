...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    short_length = min(len(branches(t1)), len(branches(t2)))
    new_branches = [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))]
    new_branches.extend([add_trees(b, tree(0)) for b in branches(t1)[short_length:]])
    new_branches.extend([add_trees(b, tree(0)) for b in branches(t2)[short_length:]])
